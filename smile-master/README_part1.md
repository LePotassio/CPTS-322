# Smile Project - CptS322 - Fall2019
...
In part 1, we will build the UI (user interface) for the app. It won't be hooked into a database yet to store the smiles you create - that will come in part 2. This part is just about learning how to build UI using semantic HTML, customize the layout, style it, and make it responsive - responsive, meaning the UI scales nicely for desktop browsers and mobile phone browsers.
...
**Important note:** If you are new to Git and haven’t used EECS GitLab server, please check out the [github_basics.txt](https://gitlab.eecs.wsu.edu/322-fall18-arslanay/smile_project_322_skeleton/blob/master/github_basic.txt) document first.
 

1. Clone the repository [smile_project_322_skeleton repository](https://gitlab.eecs.wsu.edu/322-fall19-arslanay/smile_project_322_skeleton) on the working machine
    ```
    git clone https://gitlab.eecs.wsu.edu/322-fall2019-arslanay-smile/322-fall2019-smile-skeleton.git
    ```
...
2. Next, you need to go to the repo directory and checkout all of the branches that have already been created for you.
    ```
    cd 322-fall2019-smile-skeleton/
    git checkout part1
    git checkout master
    ```

3. You now have a local copy of the `smile_project_322_skeleton` repository.
    ...
    ```
    git remote -v
    origin  https://gitlab.eecs.wsu.edu/322-fall2019-arslanay-smile/322-fall2019-smile-skeleton.git (fetch)
    origin  https://gitlab.eecs.wsu.edu/322-fall2019-arslanay-smile/322-fall2019-smile-skeleton.git (push)

    ```
    Now, you need to copy the skeleton code to you own **private** repository that you created on EECS GitLab server and commit everything to this repository. (Change the commit origin with the following command to your new repository.
...
4. Next you need to add your repository as the new origin.
    ...
    ```
    git remote add origin <URL for new repo> 
    ```
    IMPORTANT NOTE: For the warm-up project (i.e. Rate Professor), you need to add the repository we created for you "<yourlastname>-warmup" as the origin. 
...
5. Push the code to your new branch.
    ...
    ```
    cd ..
    mv 322-fall2019-smile-skeleton/ <new repo name>
    cd <new repo name>
    ```
...
1. "`Share a smile`" button for posting a new smile. 
   Add the button to the existing `<div>` with HTML class "`create`" in the `<header>`.
1. Smile post. Each post should show its:
    ...
    * **Timestamp** for when this was posted.
      Use a `<div>` with HTML class "`timestamp`", and contents of the form "Posted at 1:03pm May 22, 2019".
...
1. The logo should be centered horizontally within the header. (Hint:
   `text-align`)
1. The `<div>` that contains the `Share a smile` button should be as
   wide as the browser window. Use the HTML class "`my-button`" to style the button element. (later we will use the style definitions for the "`my-button`" for other buttons. To refer to this button only use "`share-smile`")
   The button itself should also be as wide as the browser window. (Hint:
...
1. Position the post content into three columns, so the happiness level is on
  ...
  and the like column, and set `width: calc(...)` for the story. Wrap "`happiness-level-col`", "`story-col`", and "`like-col`" in a flex `<div>` with HTML class "`smile-row`"". 
  Assign the "`display`" property of "`smile-row`" as "`flex`" and "`flex-direction`"as "`row`". )
...
1. The header (including the logo and `Share a smile` button) should be always
   ...
   scroll underneath. Also make sure that the "`top`"" position of the "`header`" is set to 0. )
...
1. To style the `Share a smile` button, add style definitions for the `"my-button"` class (which we used on the
   link element) to have: `background-color` and `border`. You may want to use slightly wider borders and slightly darker
...
1. Change the submit button's text to `Post` (i.e., value="`Post`").
...
1. Last, add a "`Cancel`" button onto the create form in case the user doesn't want to make the post. 

At this point if you open `create.html` should look like this:
...
### How to Submit
---
#### First:  Submit on GitLab
We'll use GitLab Pages to host your Smile app. To set this up:
